  ...
  useEffect(() => {
    ...
    socket.current = new WebSocket(
      window.location.protocol === 'https:' 
        ? 'wss://zengelw95chat-production.up.railway.app'
        : 'ws://zengelw95chat-production.up.railway.app'
    );
...
function handleDrop(e, name, target, oldFolderID) {
  ...
  if (droppedIcon) {
    ...
    function handleMouseSeen() { //check if user is on thefrontend
      setDetectMouse(true)
    }
...
async function getChat() {
  try {
    const response = await axios.get('https://zengelw95chat-production.up.railway.app/chat/getchat/', {
      headers: {
